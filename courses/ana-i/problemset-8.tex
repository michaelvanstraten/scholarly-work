...
\begin{enumerate}
    \item Gilt $\lim_{k\to\infty} \inf L_k > 1$, so konvergiert die Reihe
          $\sum_{k=1}^{\infty} a_k$.
    \item Existiert ein $k_0 \in \mathbb{N}$ so dass $L_k \leq 1$ für alle $k
          \geq k_0$, so divergiert die Reihe $\sum_{k=1}^{\infty} a_k$.
\end{enumerate}
\begin{proof}
    \leavevmode
    \begin{enumerate}
...
\begin{enumerate}
    ...
    \item Für jede positive reelle Zahl $a$ ist die Einschränkung $\log [a,
          \infty[ : [a, \infty[ \rightarrow \mathbb{R}$ gleichmäßig stetig.
    \item Machen Sie eine Literaturrecherche, und finden Sie die Definition von
          Lipschitzstetigkeit für reelle Funktionen heraus. Schreiben Sie diese
          nieder.
    \item Ist $\log [a, \infty[ : [a, \infty[ \rightarrow \mathbb{R}$
          lipschitzstetig?
\end{enumerate}
\begin{proof}
    \leavevmode
